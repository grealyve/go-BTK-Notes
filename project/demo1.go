...
func GetAllProducts() ([]Product, error) {
	...
	if err != nil {
		return nil, err //Array pointer mantığında çalıştığı için nil döndürebilirsin ama altta yapamazsın.
	}
...
func AddProduct() (Product, error) {
	product := Product{ProductName: "Microphone 2", CategoryId: 1, UnitPrice: 3000.0}
	jsonProduct, err := json.Marshal(product)

	response, err := http.Post("http://localhost:3000/products", "application/json;charset=utf-8", bytes.NewBuffer(jsonProduct))
	if err != nil {
		return Product{}, err //pointer değeri olmayan bir şeyi nil döndüremezsin
	}
	defer response.Body.Close()
